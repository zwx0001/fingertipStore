<template>
  <div class="report">
    ...
    <footer v-on:click="submit">提现</footer>
  </div>
</template>
<script>
...
export default {
  ...
  watch: {
    time1(n) {
      // this.start_time = n[0].toLocaleDateString().replace(/\//g, "-");
      // this.end_time = n[1].toLocaleDateString().replace(/\//g, "-");
      let arr = n[0]
        .toLocaleDateString()
        .split("/")
        .map((item, index) => {
